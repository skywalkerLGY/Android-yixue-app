...
public class MyMove extends AnimatorAdapter {

    public MyMove(CardStackView cardStackView) {
        super(cardStackView);
    }
    ...
    protected void itemExpandAnimatorSet(final CardStackView.ViewHolder viewHolder, int position) {
        ...
        for (int i = 0; i < mCardStackView.getChildCount(); i++) {
            ...
            if (i > mCardStackView.getSelectPosition() && collapseShowItemCount < mCardStackView.getNumBottomShow()) {
                ...
                ObjectAnimator oAnim = ObjectAnimator.ofFloat(child, View.Y, child.getY(), (float) (childTop*0.8));
                mSet.play(oAnim);
                collapseShowItemCount++;
            }
            else {
                ObjectAnimator oAnim = ObjectAnimator.ofFloat(child, View.Y, child.getY(), (float) ((mCardStackView.getShowHeight() )*0.8));
                mSet.play(oAnim);
            }
        }
    }
